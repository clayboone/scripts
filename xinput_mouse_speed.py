...
class xinput_mouse_speed_window(Gtk.Window):
    ...
    def __init__(self):
        ...
        self.__xinput_accel_prefix = 'xinput -set-prop 18 "libinput Accel Speed" '
        self.__xinput_speed_prefix = 'xinput -set-prop 18 '
    ...
    def create_slider(self, initial_value=0, min_value=0,
        ...
        if h_scale.get_value() is not initial_value:
            # whatever.. >.> hashtag FIXME hashtag foreveralone
    ...
    def get_initial_speed(self):
        cmd_line = "xinput -list-props 18 | grep 'Coordinate Transformation Matrix'"
        output = subprocess.check_output(cmd_line, shell=True)
        return float(output.split()[-1])

    def get_initial_accel(self):
        cmd_line = "xinput -list-props 18 | grep 'libinput Accel Speed'"
        output = subprocess.check_output(cmd_line, shell=True)
